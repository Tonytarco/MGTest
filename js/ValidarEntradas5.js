...
function dominioinvalido(objeto){
    alert(objeto);
...
function fechainvalida(objeto){

    fecha = document.getElementById(objeto); 
    if (isNaN(Date.parse(fecha.value))) 
    { 
        swal({
            title: "Formato Invalido",
            text: "Por favor escriba la fecha en el formato año-mes-dia",
            icon: "info",
            button: true
    ...
    switch (form) {
        ...
        case 'clienteP2':
            if ((blanco('Celular','')) || (tiene_pocos_char('Celular',8,12)))return false    
                ...
                else if(dominioinvalido("email"))return false
        ...
        case 'clienteP3':
         ...
         case 'usuarioP1': 
                ...
                if ((blanco('name','')) || (tiene_pocos_char("name",5,60)))return false
                    ...
                    else if(dominioinvalido("username"))return false
                      else if((blanco('password',''))||(tiene_pocos_char("password",4,30)))return false     
                        else
                        {                         
                       
                         document.forms.clientes.accion.value = "registrarusuarioP1";
                         document.forms.clientes.submit();     
                         return true
                          ...
                          if ((blanco('Celular','')) || (tiene_pocos_char('Celular',8,12)))return false    
                                    ...
                                    case 'usuarioP3':
                                      if(blanco('Perfil',''))return false
                                        ...
                                        case 'RegistrarNoticia':
                                           
                                            if ((blanco('titulo','')) || (tiene_pocos_char('titulo',3,70)))return false    
                                             else if((blanco('medio','')) || (tiene_pocos_char('medio',3,40)))return false
                                             else if((blanco('fecha','')) || (tiene_pocos_char('fecha',8,14)))return false
                                                else if(fechainvalida("fecha"))return false
                                                  else if(blanco("descripcion",''))return false
                                                   else {  
                                                          document.forms.clientes.accion.value = "RegistrarNoticia";
                                                          document.forms.clientes.submit(); 
                                                          return true;
                                                      }               
                                        case 'Cancelar':
                                                   
                                            document.forms.clientes.accion.value = form;
                                            document.forms.clientes.submit();     
                                            return true;     
                                        case 'logueo':
...
function f_revisasiguiente(){
    
       if (document.forms.clientes.name.value!=""){
        document.forms.clientes.accion.value="consulta";
        document.forms.clientes.submit();
    }
...
function Ordenar(OrderBy){
    document.forms.reporte.OrderBy.value=OrderBy;
    document.forms.reporte.submit();
}  
